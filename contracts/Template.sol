...
contract Template is TemplateBase {
    ...
    function newInstance() public {
        ...
        bytes32 burnAppId = keccak256(abi.encodePacked(apmNamehash("open"), keccak256("burn-app")));
        bytes32 tokenManagerAppId = apmNamehash("token-manager");

        Burn burn = Burn(dao.newAppInstance(burnAppId, latestVersionAppBase(burnAppId)));
        ...
        burn.initialize(tokenManager);
        emit InstalledApp(burn, burnAppId);

        /* acl.createPermission(root, burn, burn.NULL_ROLE(), root); */
        acl.createPermission(0x0, burn, 0x0, 0x0);
        acl.createPermission(this, tokenManager, tokenManager.MINT_ROLE(), this);

        tokenManager.mint(root, 1000*TOKEN_UNIT); // Give 1000 token to msg.sender

        acl.createPermission(burn, tokenManager, tokenManager.BURN_ROLE(), root);
